...
bool parallel_any(Iterator first, Iterator last, MatchType match) {
    struct find_element {
...
int main(void) {
    ...
    auto any = parallel_any(myvec.begin(), myvec.end(), [](int i){return i > 0;});
    assert(any == true);
    
    auto none = parallel_any(myvec.begin(), myvec.end(), [](int i){return i < 0;});
    assert(none == false);
}